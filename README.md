...
- 株価チャート画像の生成・投稿
- Slackへの自動投稿
- 土日・日本の祝日（東京市場休場日）は完全スキップ（Slackへも投稿しない）
...
4. **休場日でも投稿されてしまう**
  - `src/index.js` 冒頭で `getMarketClosedReason()` を確認し、理由があれば即終了します
  - 祝日判定: `japanese-holidays` + 曜日 (土日)
  - 実行環境のタイムゾーン差異で前日/当日判定がズレる場合は `Asia/Tokyo` タイムゾーンでの変換処理部分を確認してください
